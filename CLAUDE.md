...
1. **Static** — serves `index.html` at `/`, and frontend modules under `/src/*` (allowlisted extensions, path-traversal-guarded, so `data.json` and other root files are NOT exposed).
2. **Job search** (`POST /api/search`): fetches from open job APIs only — Remotive, RemoteOK, Arbeitnow (remote/EU boards), and Greenhouse/Lever/Ashby by company slug. By default also `fetchAiWebSearch` (`useWebSearch`, on by default) — shells out to `claude` with the **WebSearch tool allowlisted** (`runClaude(prompt, {allowedTools:"WebSearch"})`) to **discover companies hiring in the chosen city/country and read their career pages**, returning direct apply links; parses a JSON array, best-effort (returns `[]` if not signed in / no tool), strips LinkedIn/Indeed URLs. Every fetcher tags each job with a **`source`** string (e.g. "Remotive", "Careers · stripe", a careers domain) used as the apply reference in the UI. Each source has its own `fetch*` function (with a per-request timeout) normalizing to `{company, title, location, url, description, postedAt}`. Results are keyword-filtered, deduped by URL, **country-filtered (soft preference)**, recency-filtered (`maxAgeDays`), sorted (country matches first, then newest-first), and sliced to `limit`. **LinkedIn/Indeed are deliberately excluded** — no open API and automation risks bans; those come in manually via "Add job".
